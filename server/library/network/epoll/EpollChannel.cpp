...
bool EpollChannel::setTargetEvent(int targetEvent)
{
    ...
    bool retp =  epollWorker->updateChannel(EPOLL_CTL_MOD, m_fd, shared_from_this(), targetEvent);
	if(!retp)
	{
...
bool EpollChannel::addTargetEvent(int targetEvent)
{
    ...
    bool retp = epollWorker->updateChannel(EPOLL_CTL_ADD, m_fd, shared_from_this(), targetEvent);
	if(!retp)
	{
        m_targetEvent = event;
		LOG_FMT_ERROR(g_logger, "epoll update channel failed, socket fd = %d, errno = %d", m_fd, errno);
