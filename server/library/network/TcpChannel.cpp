...
const uint32_t TcpChannel::noneEvent = 0;
const uint32_t TcpChannel::readEvent = EPOLLIN | EPOLLHUP;
const uint32_t TcpChannel::writeEvent = EPOLLOUT;
...
bool TcpChannel::setTargetEvent(uint32_t targetEvent)
{
    auto loop = m_ownerLoop.lock();
    if(loop == nullptr)
        return false;
...
bool TcpChannel::addTargetEvent(uint32_t targetEvent)
{
    auto loop = m_ownerLoop.lock();
    if(loop == nullptr)
        return false;

    uint32_t event = m_targetEvent;
    m_targetEvent |= targetEvent;
