...
int64_t Socket::getSendTimeout() {
    FDItem::ptr ctx = FDMgr::GetInstance()->get(m_sock);
    if(ctx) {
...
bool Socket::init(int sock) {
    FDItem::ptr ctx = FDMgr::GetInstance()->get(sock);
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {
            ...
            FileSystem::Unlink(uaddr->getPath(), true);
        }
    }
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->triggerAndCancelEvent(m_sock, IOManager::EventType::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->triggerAndCancelEvent(m_sock, IOManager::EventType::WRITE);
}
...
bool Socket::cancelAll() {
    return IOManager::GetThis()->triggerAndCancelEvent(m_sock);
}
